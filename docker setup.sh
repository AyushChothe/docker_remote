...
if [ "$#" -eq 0 ] || [ "${1#-}" != "$1" ]; then
    ...
    echo "";
    echo "Modify \`ExecStart\` in \`/lib/systemd/system/docker.service\` to below and run \`systemctl daemon-reload && sudo service docker restart\`"
    echo /usr/bin/dockerd \
        --tlsverify \
...
# DOCKER_TLS_CERTDIR=~/.docker bash docker_setup.sh 
